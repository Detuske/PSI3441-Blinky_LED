...
#define LED_PINred        18       // Pino PTB18 onde está o LED vermelho
#define LED_PINgreen        19
#define LED_PINyellow        1
...
void main(void)
{
    ...
    gpio_pin_configure(port, LED_PINred, GPIO_OUTPUT_ACTIVE);
	gpio_pin_configure(port, LED_PINgreen, GPIO_OUTPUT_ACTIVE);
	gpio_pin_configure(port, LED_PINyellow, GPIO_OUTPUT_ACTIVE);

    // Configura o pino como saída ativa (LED apagado ou aceso depende de pull)

    while (1) {
        gpio_pin_toggle(port, LED_PINred);
		...
		gpio_pin_toggle(port, LED_PINgreen);
        // Inverte o nível no pino (se estava alto, passa para baixo e vice-versa)

        k_msleep(300);
        // Suspende a tarefa por SLEEP_TIME_MS milissegundos antes do próximo toggle

		gpio_pin_toggle(port, LED_PINyellow);
        // Inverte o nível no pino (se estava alto, passa para baixo e vice-versa)
